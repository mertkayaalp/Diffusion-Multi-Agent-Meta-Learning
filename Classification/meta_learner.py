...
class MetaLearner(tf.keras.models.Model):
    def __init__(self, args=None, strategy=None, bn=None):
        super(MetaLearner, self).__init__()
        # for MiniImageNet dataset set conv2d kernel size = [32, 3, 3]
        # for Omniglot dataset set conv2d kernel size = [64, 3, 3]
        if args is not None:
            if args.dataset == 'miniimagenet':
                ...
                if strategy == 'ATC' or strategy == 'CTA':
                    self.initializer = tf.keras.initializers.VarianceScaling(args.num_agents, mode='fan_avg',distribution='untruncated_normal', seed=None)
                else:
                    self.initializer = tf.keras.initializers.VarianceScaling(1.0, mode='fan_avg',distribution='untruncated_normal', seed=None)
            if args.dataset == 'omniglot':
                ...
                self.with_bn = args.with_bn
                self.training = True if args.mode is 'train' else False
                if strategy == 'ATC' or strategy == 'Centralized':
                    self.initializer = tf.keras.initializers.VarianceScaling(args.num_agents, mode='fan_avg',distribution='untruncated_normal', seed=None)
                else:
                    self.initializer = tf.keras.initializers.VarianceScaling(1.0, mode='fan_avg',distribution='untruncated_normal', seed=None)
        else:
            ...
            if bn is not None:
                ...
                self.with_bn     = False
            self.initializer = tf.keras.initializers.VarianceScaling(1.0, mode='fan_avg',
                                                                     distribution='untruncated_normal', seed=None)
        if self.with_bn is True:
            # Build model layers
            self.conv_1 = tf.keras.layers.Conv2D(filters=self.filters, kernel_size=(3,3), strides=(1,1), padding='SAME', kernel_initializer=self.initializer)
            self.bn_1 = tf.keras.layers.BatchNormalization(axis=-1)
            self.max_pool_1 = tf.keras.layers.MaxPool2D(pool_size=(2,2))

            self.conv_2 = tf.keras.layers.Conv2D(filters=self.filters, kernel_size=(3,3), strides=(1,1), padding='SAME', kernel_initializer=self.initializer)
            self.bn_2 = tf.keras.layers.BatchNormalization(axis=-1)
            self.max_pool_2 = tf.keras.layers.MaxPool2D(pool_size=(2,2))

            self.conv_3 = tf.keras.layers.Conv2D(filters=self.filters, kernel_size=(3,3), strides=(1,1), padding='SAME', kernel_initializer=self.initializer)
            self.bn_3 = tf.keras.layers.BatchNormalization(axis=-1)
            self.max_pool_3 = tf.keras.layers.MaxPool2D(pool_size=(2,2))

            self.conv_4 = tf.keras.layers.Conv2D(filters=self.filters, kernel_size=(3,3), strides=(1,1), padding='SAME', kernel_initializer=self.initializer)
    ...
    def meta_update(cls, model, args, alpha=0.01, grads=None):
        # Make a hard copy of the target model, if with bn layers, call like copied_model = cls(bn=True)
        copied_model = cls(args)
        '''
        ...
        if copied_model.with_bn is True:
            ...
            # If calling with gradients, apply it by using SGD
            # Manually apply Gradient descent as the task-level optimizer
            if grads is not None:
                copied_model.conv_1.kernel = copied_model.conv_1.kernel - alpha * grads[0]
                copied_model.conv_1.bias = copied_model.conv_1.bias - alpha * grads[1]
